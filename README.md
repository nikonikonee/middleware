...
6. **Split keys into scattered chunks.** Each of the three key blobs (K1, K2, middle-AES key) is cut into N chunks (default 8). Each chunk is stored as a numeric-keyed entry in an `Object.create(null)` dictionary. At decrypt time the bootstrap does `Object.keys(...).sort()` to rebuild the key.
